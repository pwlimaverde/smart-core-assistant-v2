...
class _AuditPageState extends State<AuditPage> {
  ...
  void _goToPreviousPage() {
    ...
    setState(
      () => _currentOffset = (_currentOffset - _pageSize).clamp(
        0,
        _currentOffset,
      ),
    );
  ...
  Future<void> _exportCsv() async {
    ...
    if (res case Success(:final value)) {
      _downloadFile(
        value,
        'tenants_export_${DateTime.now().millisecondsSinceEpoch}.csv',
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Erro ao exportar CSV: ${ErrorMessageMapper.map(error)}',
          ),
            ...
            Row(
              ...
              children: [
                ...
                ElevatedButton.icon(
                  ...
                  label: Text(
                    _isExporting ? 'Exportando...' : 'Exportar Tenants (CSV)',
                  ),
            ...
            Expanded(
              child: ViewStateBuilder<AuditController, List<AuditLogEntry>>(
                ...
                onError: (context, error) =>
                    AppErrorView(message: error.message, onRetry: _fetchPage),
                onSuccess: (context, logs) {
                  if (logs.isEmpty) {
                    return const Center(
                      child: Text(
                        'Nenhum log de auditoria encontrado para o filtro aplicado.',
                      ),
                    );
                  }
