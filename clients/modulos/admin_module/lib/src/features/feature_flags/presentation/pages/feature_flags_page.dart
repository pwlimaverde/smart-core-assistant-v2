...
class _FeatureFlagsPageState extends State<FeatureFlagsPage> {
  ...
  Future<void> _toggleGlobalFlag(FeatureFlag flag, bool value) async {
    final res = await _controller.setFeatureFlag(
      key: flag.key,
      enabledGlobally: value,
    );
    if (!mounted) return;
    if (res case Failure(:final error)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Erro ao atualizar flag: ${ErrorMessageMapper.map(error)}',
          ),
  ...
  Future<void> _removeOverride(String flagKey, String tenantId) async {
    ...
    if (res case Failure(:final error)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Erro ao remover override: ${ErrorMessageMapper.map(error)}',
          ),
          backgroundColor: Colors.red,
        ),
      );
  ...
  void _showAddOverrideDialog(FeatureFlag flag) {
    if (_allTenants.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Nenhum tenant carregado para overrides.'),
        ),
      );
      return;
    }
    ...
    final availableTenants = _allTenants
        .where((t) => !existingTenantIds.contains(t.id))
        .toList();
    ...
    if (availableTenants.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Todos os tenants já possuem override configurado para esta flag.',
          ),
        ),
      );
      return;
                  ...
                  onPressed: () async {
                    ...
                    if (res case Failure(:final error)) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            'Erro ao salvar override: ${ErrorMessageMapper.map(error)}',
                          ),
                          backgroundColor: Colors.red,
                        ),
                      );
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Expanded(
              child:
                  ViewStateBuilder<FeatureFlagsController, List<FeatureFlag>>(
                          ...
                          itemBuilder: (context, oIndex) {
                            ...
                            return ListTile(
                              ...
                              trailing: IconButton(
                                ...
                                onPressed: () =>
                                    _removeOverride(flag.key, o.tenantId),
                              ),
                            );
                          },
                        ),
