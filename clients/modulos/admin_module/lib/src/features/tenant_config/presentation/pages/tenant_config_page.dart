...
class _TenantConfigPageState extends State<TenantConfigPage>
          ...
          children: [
            ...
            if (_hasFetched)
              ...
              const Expanded(
                child: Center(
                  child: Text(
                    'Informe o ID de um Tenant e clique em Carregar.',
                  ),
                ),
              ),
          ],
    ...
    return SingleChildScrollView(
      child: Column(
        children: [
          ...
          const SizedBox(height: 16),
          AppTextField(label: 'Modelo de Visão', controller: _visionModelCtrl),
          const SizedBox(height: 16),
          AppTextField(
    ...
    if (mounted) {
      if (res case Failure(:final error)) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Erro ao salvar configurações: ${ErrorMessageMapper.map(error)}',
            ),
          ),
        );
      } else {
