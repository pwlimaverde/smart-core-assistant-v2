...
class _TenantsPageState extends State<TenantsPage> {
  ...
  void _toggleActive(String id, bool active) async {
    ...
    if (res case Failure(:final error)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Erro ao alterar status: ${ErrorMessageMapper.map(error)}',
          ),
        ),
      );
    }
  ...
  void _generateAccessCode(String id) async {
    ...
    if (mounted) {
      if (res case Success(:final value)) {
        showDialog(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                const Text(
                  'Utilize este código para autenticação de suporte ou onboarding:',
                ),
        ...
        _controller
            .fetchTenants(); // recarrega para mostrar o código atualizado
      } else if (res case Failure(:final error)) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Erro ao gerar código: ${ErrorMessageMapper.map(error)}',
            ),
          ),
        );
      }
  ...
  void _showEditDialog(BuildContext context, [Tenant? tenant]) {
    ...
    final ownerController = TextEditingController(
      text: tenant != null ? '${tenant.ownerId}' : '',
    );
