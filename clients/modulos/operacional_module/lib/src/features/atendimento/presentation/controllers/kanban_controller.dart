...
/// snapshot anterior e devolve o [AppError] para a UI exibir (ex.: snackbar).
///
...
final class KanbanController extends BaseController<KanbanViewModel> {
  ...
  /// Serviço de domínio usado para assinar o stream realtime (opcional —
  /// testes de unidade do controller não precisam abrir stream).
  final AtendimentoService? service;
  ...
  KanbanController({
    ...
    final service0 = service;
    if (service0 != null) _assinarStream(service0);
  }
  ...
  void _assinarStream(AtendimentoService service) {
    _streamSubscription = service.streamAtendimentos().listen(
      (_) {
  ...
  Future<ReturnSuccessOrError<KanbanViewModel>> _fetchViewModel(
    int? departamentoId,
  ) async {
    final res = await _listUsecase.call(departamentoId: departamentoId);
    if (res case ErrorReturn<List<AtendimentoResumo>>(:final result)) {
      return ErrorReturn(error: result);
    }
    final atendimentos = (res as SuccessReturn<List<AtendimentoResumo>>).result;
    return SuccessReturn(
      success: KanbanViewModel(
        departamentoId: departamentoId,
        porEtapa: KanbanViewModel.agruparPorEtapa(atendimentos),
      ),
    );
  }
  ...
  /// desfaz o movimento local e devolve o [AppError] para a UI exibir, sem
  /// perder o restante do estado carregado.
  Future<AppError?> moverCard({
    ...
    final res = await _moveUsecase.call(
      atendimentoId: atendimentoId,
      etapaDestinoId: etapaDestinoId,
    );

    if (res case ErrorReturn<Unit>(:final result)) {
