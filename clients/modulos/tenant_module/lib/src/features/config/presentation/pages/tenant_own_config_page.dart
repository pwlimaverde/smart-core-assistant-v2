...
import '../widgets/config_avancada_form.dart';
    ...
    return AppScaffold(
      ...
      body: Padding(
        ...
        child: ViewStateBuilder<TenantOwnConfigController, TenantConfig>(
          ...
          onSuccess: (context, config) => SingleChildScrollView(
            child: Column(
              children: [
                _ConfigForm(
                  config: config,
                ...
                if (_controller.podeEditarAvancada) ...[
                  const SizedBox(height: 24),
                  ConfigAvancadaForm(
                    // Recria o formulário quando a config recarrega.
                    key: ObjectKey(config),
                    avancada: config.avancada,
                    podeSalvar: sessaoPodeAlterar('/tenant/config'),
                    aoSalvar: (avancada, removidos) async {
                      final res = await _controller.updateAvancada(
                        avancada,
                        promptsRemovidos: removidos,
                      );
                      return switch (res) {
                        Failure(:final error) =>
                          'Erro ao salvar: ${ErrorMessageMapper.map(error)}',
    ...
    return AppCard(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Transcrição: ${widget.config.transcriptionProvider} / ${widget.config.transcriptionModel}',
          ),
          Text(
            'Visão: ${widget.config.visionProvider} / ${widget.config.visionModel}',
          ),
          const SizedBox(height: 24),
          Text(
