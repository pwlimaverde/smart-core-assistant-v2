...
mod tests {
    ...
    // Teste de integração do consumidor de auditoria (função privada que persiste no
    // Postgres real). Fica `#[ignore]` para manter o caminho rápido `--bins` sem túnel
    // SSH; execute com `cargo test -p data_postgres -- --ignored` contra um banco real.
    #[tokio::test]
    #[ignore = "requer Postgres real; rode com --ignored"]
