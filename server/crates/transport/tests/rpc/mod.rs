...
async fn test_rpc_flow_success() {
    ...
    let client = MuxClient::conectar(endpoint, codec).await.expect("Falha ao conectar cliente");
    ...
    let response = client.call(request, Duration::from_secs(2)).await.expect("Falha na chamada RPC");
...
async fn test_rpc_flow_auto_reconnect() {
    ...
    let listener = tokio::net::TcpListener::bind("127.0.0.1:28494").await.unwrap();
    ...
    let listener_handle = tokio::spawn(async move {
        ...
        // Escreve a resposta
        transport::framing::write_frame(&mut stream, &resp_frame).await.unwrap();
    ...
    let resp1 = client.call(request.clone(), Duration::from_secs(2)).await.unwrap();
    ...
    let server2 = Server::new(endpoint.clone(), "flatbuffers")
        .route("Echo", |env| {
            Box::pin(async move {
                Envelope {
    ...
    let resp2 = client.call(request, Duration::from_secs(5)).await.expect("Deveria ter reconectado automaticamente");
